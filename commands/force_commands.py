...
class Force:
    ...
    def createInfoScreen(self):
        ...
        imagePath = os.path.join(imageFolder, "force screen no-text labels.png")
        ...
        force = self
        nameSize, namebbox = ImageGeneration.getFontSize(blockfontPath,force.Name,290,45)
        wrapWidth = 30
        ...
        descSize,descbbox = ImageGeneration.getFontSize(playfontPath,description,340,375,minSize=12)
        ...
        with Image.open(imagePath).convert("RGBA") as forceScreen:
            ...
            draw.text((485,nameycentered),force.Name, font = nameFont, fill='#36CFCA')
            draw.text((60,95),description, font = descFont, fill='#62FBDF')
            if len(force.Image)>0:
                forceImg = Image.open(BytesIO(requests.get(force.Image).content)).convert("RGBA")
                ...
                forceScreen.alpha_composite(resized_img, (int(475+(320-new_size[0])/2),235))
    ...
    def createMemberScreen(self):
        ...
        generatedImagePath = os.path.join(imageGeneratedFolder,"force-members.png")
        fontFolder = os.path.join(os.path.dirname( __file__ ), os.pardir, 'fonts')
        blockfontPath = os.path.join(fontFolder, "Blockletter.otf")
        playfontPath = os.path.join(fontFolder, "Play-Regular.ttf")
        force = self
        ...
        with Image.open(imagePath).convert("RGBA") as forceScreen:
            for role in sortedMembers.keys():
                roleSize, rolebbox = ImageGeneration.getFontSize(blockfontPath,role,92,33)
                ...
                for member in sortedMembers[role].keys():
                    ...
                    nameSize, namebbox = ImageGeneration.getFontSize(blockfontPath,member,188,33)
                    nameFont = ImageFont.truetype(blockfontPath, size= nameSize)
                    ...
                    draw = ImageDraw.Draw(forceScreen)
                    draw.text((xrole,yrole),role, font = roleFont, fill='#36CFCA')
                    draw.text((xname,yname),member, font = nameFont, fill='#62FBDF')
                    forceScreen.save(generatedImagePath)
                    slot+=1
        return discord.File(generatedImagePath)
    def save(self):
    ...
    def createPageView(self,type=None):
        ...
        numofPages = 0
        if self.Members != []:
            view.add_item(ForceNavButton(force=self,label="Members",page="characters",display_type=type))
            numofPages+=1
        if numofPages>0:
            view.add_item(ForceNavButton(force=self,label="Info",page="info",display_type=type))
        return view
    @staticmethod
...
class ForceNest:
    ...
    def __init__(self, Name: str, Link: str, Force: Force,Size: str, Desc: str = None, Image: str = None, Id: int = None):
        self.Name = Name
        self.Link = Link
        self.Force = Force
        self.Desc = Desc
    ...
    def to_dict(self):
        exclude=["self","Force"]
        dict = {attr: getattr(self, attr) for attr in dir(self) if not callable(getattr(self, attr))and not attr.startswith("__") and attr not in exclude}
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.display_type =="Image":
            if self.page.casefold() == "characters":
                await response.edit_message(file=force.createMemberScreen(),attachments=[])
            if self.page.casefold() == "info":
                await response.edit_message(file=force.createInfoScreen(),attachments=[])
        else:
            if self.page.casefold() == "characters":
...
class ForceNestModal(Modal):
    def __init__(self, oldValues:ForceNest =None, edit=False,force:Force=None,size: str = "tiny", *args, **kwargs) -> None:
        self.edit = edit
        self.force = force
        self.size = size
        if not edit:
            ...
            self.add_item(InputText(label="Description", placeholder="describe the force nest here", style=discord.InputTextStyle.long,row=1,max_length=1800))
            ...
            self.add_item(InputText(label="Description", placeholder="describe the force nest here", style=discord.InputTextStyle.long,row=1,max_length=1800,value = oldValues.Desc))
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.edit:
            ...
            if self.oldValues != None:
                ...
                await interaction.response.send_message(f"No forcenest found: {name}",ephemeral=True)
        else:
            newForceNest = ForceNest(
                Name= name,
                Link= googledoc,
                Force= self.force,
                Desc= desc,
...
class ForceModal(Modal):
    def __init__(self, oldValues=None, edit=False, *args, **kwargs) -> None:
        ...
        if not edit:
            super().__init__(*args, **kwargs)
            self.add_item(InputText(label="Name", placeholder="Put the name here", style= discord.InputTextStyle.short,row=0))
            self.add_item(InputText(label="Description", placeholder="describe the form here", style=discord.InputTextStyle.long,row=1,max_length=1800))
            self.add_item(InputText(label="Image", placeholder="put a link to an image here", style=discord.InputTextStyle.short,row = 2 ,required=False))
            self.add_item(InputText(label="Document", placeholder="put a link to a google doc or discord message link here", style=discord.InputTextStyle.short, row= 3))
            ...
            self.add_item(InputText(label="Image", placeholder="put a link to an image here", style=discord.InputTextStyle.short,row = 2 ,required=False,value=oldValues.Image))
            self.add_item(InputText(label="Document", placeholder="put a link to a google doc or discord message link here", style=discord.InputTextStyle.short, row= 3, value=oldValues.Link))
            self.add_item(InputText(label="Color", placeholder="put the colour associated with the force in hexademical form ex: ffffff would be white",value=oldValues.Colour,min_length=6,max_length=6, style=discord.InputTextStyle.short, row= 4))
            self.oldValues = oldValues
    async def callback(self, interaction: discord.Interaction):
        googledoc = self.children[3].value
        desc = self.children[1].value
        ...
        if self.edit:
            ...
            if interaction.guild_id != dbForce.ServerID:
                ...
                await interaction.response.send_message(f"{name} does not belong to this server. Please edit the force on its coressponding server",ephemeral=True)
                return
            if dbForce != None:
                print(dbForce)
            ...
            embed = newForce.createEmbed(leader)
            await interaction.response.send_message(f"{name} has been created!", embed=embed)
...
class ForceCommands(commands.Cog):
    ...
    nestCommandGroup = force.create_subgroup('nest', "Commands to add or edit a force's nest")

    @nestCommandGroup.command(guild_ids=[*guildids], description='Create a Force Nest', name = "create")
    async def nestcreate(self, ctx, 
        force: Option(str,'the force you want to add the nest to' ,required=True),
        size: Option(str,'The size of the Force nest', choices=["Tiny",'Small','Meduim','Large'],required=True)
    ):
        dbForce = Force.searchDatabase(force)
        if dbForce != None:
            modal = ForceNestModal(title="Create a Force Nest",force=dbForce,size=size)
    ...
    async def edit(
        ...
        if dbForce != None:
            if ctx.author.id == dbForce.Leader:
                modal = ForceModal(title=f"Edit {dbForce.Name}.",edit=True,oldValues=dbForce)
                await ctx.send_modal(modal)
            else:
